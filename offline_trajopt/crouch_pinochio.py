...
def solve_pinocchio():
    ...
    final_vel = vs[-(n_flight)][0:3]
    v_launch_target = np.array([0, 0, 1.2 * v_z_required]) # adding some buffer to velocity for stability
    ...
    opti.subject_to(vs[0] == 0) # Start with zero velocity
    opti.subject_to(final_vel == v_launch_target) # end with launch velocity
    opti.subject_to(qs[0] == q_nom)
    opti.subject_to(qs[-1][2] >= 0.80) # final z location in the air
    #opti.subject_to(qs[-1][2] == z_crouch)
    opti.subject_to(qs[k_bottom][2] <= z_crouch)
    ...
    for k in range(N):
        ...
        if k < N - 1:
            ...
            for f_idx in range(2):
                ...
                for c_idx in range(4):
                    ...
                    # applied individually to each corner
                        ...
                        opti.subject_to(p_corner_world[2] >= 0.00)
            ...
            if k < k_bottom:
                opti.subject_to(qs[k+1][2] <= qs[k][2])
            elif k < N - n_flight:
                opti.subject_to(qs[k+1][2] >= qs[k][2])
    ...
    for k in range(N):
        ...
        cost += w_base_pos_x * cas.sumsqr(qs[k][0] - q_target[0]) # Keep base near origin
        cost += w_base_pos_y * cas.sumsqr(qs[k][1] - q_target[1]) # Keep base near origin
        cost += w_base_pos_z * cas.sumsqr(qs[k][2] - target_z) # Keep base near target height
    ...
    for k in range(N):
        if k < k_bottom:
            alpha = k / k_bottom
            z_guess = (1 - alpha) * z_stand + alpha * z_crouch
            # Create a guess that moves linearly
            q_guess = pin.interpolate(model, q_nom, q_crouch_pose, alpha)
        else:
            alpha = (k - k_bottom) / (N - 1 - k_bottom)
            z_guess = (1 - alpha) * z_crouch + alpha * z_stand
            q_guess = pin.interpolate(model, q_crouch_pose, q_nom, alpha)

        q_guess[2] = z_guess
        opti.set_initial(qs[k], q_guess)
        opti.set_initial(vs[k], np.zeros(model.nv))

    for k in range(N-1):    
        opti.set_initial(dt[k], T_total/N)

    force_guess_z = weight / 8.0
    ...
    for k in range(N-1):
        for f_idx in range(2):
            for c_idx in range(4):
                # Guess [0, 0, weight/8]
                if k < N - n_flight:
                    opti.set_initial(fs[k][f_idx][c_idx], np.array([0, 0, force_guess_z]))
                else:
