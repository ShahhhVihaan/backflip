...
def solve_pinocchio():
    ...
    opti.subject_to(qs[-1][3] == 0)
    opti.subject_to(qs[-1][4] == 0)
    opti.subject_to(qs[-1][5] == 0)
    opti.subject_to(qs[-1][6] <= -0.9)
    ...
    for k in range(N):
        ...
        if k == k_apex:
            ...
            opti.subject_to(qs[k][6] < 0.1)  # w close to 0
            opti.subject_to(qs[k][6] > -0.1)
        if k == 0 or k == N:
            ...
            for f_idx in range(2):
                ...
                for c_idx in range(4):
                    ...
                    # applied individually to each corner
                        ...
                        opti.subject_to(f_corner[2] <= 2.0 * weight/4)
    ...
    W_joints_air = cas.MX(np.ones(n_actuated) * 5.0) 
    w_base_rot = 30.0 
    w_force = 1e-2
    w_force_rate = 1e-4
    # Minimize joint velocities (smoothness)
    for k in range(N):
        cost += 1e-3 * cas.sumsqr(vs[k])    
    ...
    for k in range(N-1):
        for f_idx in range(2):      # Loop over 2 feet
            ...
            for c_idx in range(4):  # Loop over 4 corners
                ...
                if k < k_liftoff:
                    f_corner_next = fs[k+1][f_idx][c_idx]
                    cost += w_force_rate * cas.sumsqr(f_corner_next - f_corner_current)
    ...
    for k in range(N):
        ...
        current_w_rot = w_base_rot
        current_joint_w = W_joints
        if k<k_liftoff:  
            if k < k_bottom:
                progress = k / k_bottom
        ...
        cost += cas.sum1(current_joint_w * q_diff**2)
        cost += current_w_rot * cas.sumsqr(qs[k][3:7] - q_nom[3:7])
    ...
    for k in range(N):
        if k < k_bottom:
            ...
            qy_rot = np.sin(angle / 2)
            qw_rot = np.cos(angle / 2)
        ...
        if k < N-1:
            if k_bottom < k < k_liftoff:
                ...
                v_guess = np.zeros(model.nv)
                thrust_progress = (k - k_bottom) / (k_liftoff - k_bottom)
                v_z_guess = 3.5 * thrust_progress
                v_guess[2] = v_z_guess
                v_guess[4] = -4.0
                opti.set_initial(vs[k], v_guess) # Guess 2 m/s upwards
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(results_dir / "total_forces_flip.png", dpi=300)
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(results_dir / "corner_forces_flip.png", dpi=300)

    L_world = np.zeros((N, 3))
    v_com_sol = np.zeros((N, 3))
    print("Computing angular momentum for plot...")
    for k in range(N):
        # Update Pinocchio data with solution
        pin.computeCentroidalMomentum(model, data, q_sol[k], v_sol[k])
        
        # Extract Angular Momentum (L)
        # pin.data.hg contains [Linear, Angular] 6D Wrench
        L_world[k, :] = data.hg.angular
        lin_mom = data.hg.linear
        v_com_sol[k, :] = lin_mom / total_mass

    plt.figure(figsize=(10, 6))
    
    # Plot Lx, Ly, Lz
    plt.plot(t_array_full, L_world[:, 0], label='Lx (Roll)', color='red', alpha=0.5, linestyle='--')
    plt.plot(t_array_full, L_world[:, 1], label='Ly (Pitch - FLIP)', color='blue', linewidth=2)
    plt.plot(t_array_full, L_world[:, 2], label='Lz (Yaw)', color='green', alpha=0.5, linestyle='--')
    
    # Draw vertical lines for phases
    plt.axvline(x=t_array_full[k_liftoff], color='k', linestyle=':', label='Liftoff')
    plt.axvline(x=t_array_full[k_touchdown], color='k', linestyle=':', label='Touchdown')
    
    plt.title("Centroidal Angular Momentum (World Frame)")
    plt.xlabel("Time (s)")
    plt.ylabel("Angular Momentum (kg * m^2 / s)")
    plt.legend()
    plt.grid(True)
    plt.savefig(results_dir / "angular_momentum_flip.png", dpi=300)
    
    plt.figure(figsize=(10, 6))
    
    # Plot CoM Z Velocity (Vertical Jump Speed)
    plt.plot(t_array_full, v_com_sol[:, 2], label='CoM Z (Vertical)', color='blue', linewidth=2)
    
    # Plot CoM X Velocity (Forward Speed)
    # Important for Flip: You should see this stay constant during flight!
    plt.plot(t_array_full, v_com_sol[:, 0], label='CoM X (Forward)', color='green', linestyle='--')

    # Draw vertical lines for phases
    if 'k_liftoff' in locals():
        plt.axvline(x=t_array_full[k_liftoff], color='k', linestyle=':', label='Liftoff')
    if 'k_touchdown' in locals():
        plt.axvline(x=t_array_full[k_touchdown], color='k', linestyle=':', label='Touchdown')

    plt.title("Center of Mass Velocity (World Frame)")
    plt.xlabel("Time (s)")
    plt.ylabel("Velocity (m/s)")
    plt.legend()
    plt.grid(True)    
    plt.savefig(results_dir / "com_velocity_profile_flip.png", dpi=300)
    ...
    pkl_filename = "trajectory_solution_flip.pkl"
